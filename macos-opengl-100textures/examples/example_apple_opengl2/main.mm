...
-(void)updateAndDrawDemoView
{
    ...
    glClearColor(fmod(time, 1.0f), 0.0f, 0.0f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT);

    glUseProgram(program);
    float across = floor(sqrt(num));
    float down = floor(num / across);
    for (int i = 0; i < num; ++i) {
        float x = fmod(float(i), across);
        float y = floor(float(i) / across);
        float u = x / across * float(width);
        float v = y / down * float(height);
        glViewport(u, v, 10, 10);
        glBindTexture(GL_TEXTURE_2D, textures[i]);
        glBindFramebuffer(GL_FRAMEBUFFER, framebuffers[(i + 1) % num]);
