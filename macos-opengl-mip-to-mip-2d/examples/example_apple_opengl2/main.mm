...
-(void)initialize
{
  ...
  glAttachShader(program2,
                 compileShader(GL_FRAGMENT_SHADER, R"(#version 410
                                                    ...
                                                    out vec4 fragColor;
                                                    void main() {
                                                        fragColor = texelFetch(tex, ivec2(0, 0), 0).bgra;
                                                    })"));
  ...
  uint8_t colorMip1[] = {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, &colorMip0);
    glTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, &colorMip1);
